...
def check_attendance(request):
    ...
    try:
        ...
        if existing_attendance.exists():
            ...
            return render(request, 'attend/webcam.html', {'wid': '', 'confidence': '', 'wname': worker.wname, 'msg': wid[0]+' | 출근 기록 있음'})
            ...
            return render(request, 'attend/webcam.html', {'wid': wid[0], 'confidence': confidence[0], 'wname': worker.wname,'msg': wid[0]+' | 출근 기록 완료', 'code':check})
...
def check_leave(request):
    ...
    try:  
        ...
        if not attendance_data.leave_time:
            ...
            return render(request, 'attend/leave.html', {'wid': wid[0], 'confidence': confidence[0], 'wname': worker.wname, 'msg': f'{datetime.now().strftime("%Y-%m-%d %H:%M:%S")} :: {wid[0]} | 퇴근 기록 완료'})
        
        # 이미 퇴근 완료한 경우
        else:  
            print(f'{wid[0]} >> 이미 퇴근 완료함')
            return render(request, 'attend/leave.html', {'wid': '', 'confidence': '', 'wname': worker.wname, 'msg': f'{wid[0]} | 퇴근 기록 있음'})

    except Attendance.DoesNotExist:
        # 출근하지 않은 작업자인 경우
        print(f'Attendance Error == {wid[0]}, 출근하지 않은/없는 작업자 입니다.')
        return render(request, 'attend/leave.html', {'wid': '', 'confidence': '', 'msg': f'{wid[0]} | Wrong ID'})
