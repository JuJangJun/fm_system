...
########################################################################################
    ...
    def calculate(image):
        ...
        for i in range(len(bboxes)): #탐지된 박스 수만큼 돌아갈것이다.
            ...
            elif bboxes[i][-1] == 4.0: #조끼 (몸통박스와 비교)
                print("여기는 조끼(3) : ",bboxes[i][-1])
        ...
        print(f"openpose까지의 시간 : {media_pipe-start}")
        print(f"yolo까지의 시간 : {yolo-media_pipe}")
